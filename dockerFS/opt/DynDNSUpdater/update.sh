...
echo "INFO Current IP is: $CURRENT_IP"

# Load 'old' IP
if [[ -e $IP_STORE ]]; then
	STORED_IP=$(cat $IP_STORE)
	echo "INFO Stored IP is: $STORED_IP"
elif [[ ! -e $IP_STORE ]]; then
	# Create the file to store the IP inside
	touch $IP_STORE
	echo "INFO No IP previously stored, creating IP Store"
fi;
...
if [[ ! $STORED_IP == $CURRENT_IP ]]; then
	...
	echo "INFO Answer from DynDNS-Update-Service: $ANSWER"
	echo $CURRENT_IP > $IP_STORE
	echo "INFO Saved current IP into IP Store"
elif [[ $STORED_IP == $CURRENT_IP ]]; then
	echo "INFO IP Adresses match, no update is required"
fi;
